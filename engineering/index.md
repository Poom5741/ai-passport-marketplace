# engineering/index.md

| # | task folder | title | status |
|---|-------------|-------|--------|
| 1 | 0001-cloudflare-images | Cloudflare Images setup | construct |
| 2 | 0002-project-tags | project_tags DDL | construct |
| 3 | 0003-bcrypt-cost | bcrypt cost 10 | construct |
| 4 | 0004-worker-proxy | Worker proxy upload | construct |
| 5 | 0005-waituntil-views | waitUntil view counts | construct |
| 6 | 0006-auth-tests | Auth unit tests | construct |
| 7 | 0007-project-tests | Project submission tests | construct |
| 8 | 0008-e2e-tests | E2E tests | construct |
| 9 | 0009-tag-tests | Tag normalization tests | construct |

> Note: this is a greenfield scaffold. All code is written and unit tests pass.
> inspect/release phases pending Cloudflare account setup and CI configuration.
